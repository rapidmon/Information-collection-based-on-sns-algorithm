...
## 중요도 기준 (두 축으로 평가, 반드시 차등 부여)

중요도는 "뉴스 가치"와 "실무 활용도" 두 축 중 **높은 쪽**을 최종 점수로 채택하세요.
산업 빅뉴스가 아니더라도 실무에 바로 쓸 수 있는 정보라면 높은 점수를 부여합니다.
...
### 축 2: 실무 활용도 (업무 기획·개발에 실제로 쓸 수 있는가)
- 0.8-1.0: 바로 업무에 도입하거나 기획에 반영할 수 있는 도구/기술/사례 (예: 실전 활용 가능한 AI 도구·오픈소스 프로젝트, 구체적 구현 데모, 새 API·SDK 출시)
- 0.6-0.79: 기획 아이디어나 방향성에 참고할 수 있는 정보 (예: 특정 기술 실사용 후기, A/B 테스트 결과, 워크플로우 개선 사례, 프로덕트 설계 인사이트)
- 0.4-0.59: 배경 지식으로는 유용하지만 당장 적용은 어려운 정보
- 0.0-0.39: 실무 적용과 무관한 단순 소식/의견"""
...
## 중요도 판단 예시 (뉴스 가치 + 실무 활용도 중 높은 쪽 채택)
- "OpenAI, GPT-5 출시" → 0.95 (산업 판도 변화)
- "삼성전자 HBM3E 양산 시작" → 0.85 (주요 기술 발표)
- "대학생이 10일 만에 만든 AI 시뮬레이션 도구 오픈소스 공개" → 0.8 (실무 활용 가능한 도구)
- "LangGraph로 멀티에이전트 워크플로우 구축하는 구체적 방법" → 0.75 (실전 구현 사례)
- "GitHub Copilot 새 기능 추가" → 0.6 (의미있는 업데이트)
- "특정 프롬프트 기법으로 응답 품질 30% 향상시킨 후기" → 0.65 (실무 참고 가능)
...
CROSS_CHUNK_MERGE = """아래는 청크별로 이미 중복 제거된 토픽 목록입니다. 서로 다른 청크에서 **동일한 구체적 사건/뉴스**를 다루는 토픽이 있으면 하나로 병합하세요.

## 규칙
- **같은 구체적 사건/발표**를 다루는 토픽만 병합하세요.
- 카테고리나 기업명만 같은 것은 병합하지 마세요.
- 병합 시 두 토픽의 body_bullets를 합치고, post_ids/sources/source_urls도 모두 합치세요.
- 병합 대상이 없는 토픽은 그대로 유지하세요.
- headline은 더 구체적이고 정보가 풍부한 쪽을 채택하세요.
- importance_score는 더 높은 쪽을 채택하세요.

## 토픽 목록
{topics_json}

## 출력 형식 (병합할 토픽의 인덱스 쌍 배열)
병합할 토픽이 없으면 빈 배열을 반환하세요.
[
  {{
    "merge_indices": [0, 5, 12],
    "headline": "병합된 헤드라인",
    "importance_score": 0.92
  }}
]"""

DEDUPLICATE_AND_MERGE = """아래 게시물 목록에서 **동일한 구체적 사건/발표**를 다루는 항목들만 그룹으로 묶고, 각 그룹에 대해 통합 브리핑 항목을 생성하세요.

## 그룹핑 핵심 규칙 (반드시 엄격히 준수)
- **같은 구체적 사건/발표/뉴스**를 다루는 게시물만 묶으세요.
- 단순히 같은 카테고리(AI, 반도체 등)이거나 같은 기업 이름이 언급된다고 묶지 마세요.
- 예시:
  - ✅ 묶어야 함: "OpenAI GPT-5 출시" + "GPT-5 벤치마크 결과" (같은 사건)
  - ❌묶으면 안 됨: "OpenAI GPT-5 출시" + "OpenAI 조직 개편" (다른 사건)
  - ❌ 묶으면 안 됨: "Nvidia GPU 매출 증가" + "TSMC 3nm 양산" (둘 다 반도체지만 다른 사건)
- 관련이 없는 게시물은 **반드시 개별 항목**으로 출력하세요. 억지로 묶지 마세요.

## 작업
1. 동일한 구체적 사건을 다루는 게시물만 그룹으로 묶으세요.
2. 각 그룹(또는 단독 게시물)에 대해:
...
[
  {{
    ...
    "source_urls": ["https://twitter.com/...", "https://dcinside.com/..."]
  }}
]"""
