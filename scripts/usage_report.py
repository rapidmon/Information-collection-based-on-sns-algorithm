...
두 CLI 백엔드가 같은 포맷으로 남기는 줄을 합산한다:
    Codex CLI (codex_cli_processor._call_api):
        [usage] model=... in=... out=... (cached=... reasoning=...)
    Claude CLI (claude_code_processor._run_claude):
        [usage] model=... in=... out=... (cache_read=... cache_write=...)

두 CLI 모두 현금 종량 과금이 아니라 구독 한도 소비다. 이 스크립트는 그 소비가
'어디서' 나왔는지를 쪼개 보여주는 용도다.
...
LINE = re.compile(
    ...
    r"(?:\s+\(cached=(?P<cached>\d+)\s+reasoning=(?P<codex_reasoning>\d+)\))?"
)
...
def main() -> int:
    ...
    with path.open(encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            for k in _KEYS:
                value = m.group(k)
                if k == "reasoning":
                    value = value or m.group("codex_reasoning")
                elif k == "cache_read":
                    value = value or m.group("cached")
                s[k] += int(value or 0)

    if not stats:
        print("[usage] 줄이 없다.")
        print("계측은 codex_cli_processor._call_api / claude_code_processor._run_claude 에 있다"
              " — 로그 경로를 확인할 것.")
        return 1
    ...
    print()
    print("해석 가이드")
    print("  · gpt-*와 claude-* 줄 모두 CLI 구독 한도 소비다.")
